...
def calculate_channel_busy_ratio(df):
...
data = [calculate_channel_busy_ratio(df1), calculate_channel_busy_ratio(df2)]
labels = ["ETSI", "RRS"]
colors = ["blue", "orange"]
# print(bins, times)
fig, ax = plt.subplots()
for i in range(len(data)):
    ax.plot(data[i][0], data[i][1], color=colors[i], label=labels[i])
ax.set(xlabel='Simulation Time (s)', ylabel="CBR")
ax.legend(loc="lower left")
ax.tick_params(direction='in')
ax.set_xlim([0, (max(data[0][0]) + 1)])
ax.set_ylim([0, 1.1])
plt.xticks(np.arange(0, (max(data[0][0]))+5, step=5))
plt.yticks(np.arange(0, 1.1, step=0.1))
plt.savefig(args[1] + args[2] + "_CBR2.png", dpi=300)
# plt.show()
plt.close(fig)
