...
if __name__ == "__main__":
    ...
    for i in range(len(cpmMethodLabels)):
        for j in range(len(penetrationLabels)):
            ...
            times, cbr = getCBR(dfs[k])
            ax.plot(times, cbr, ls=lines[i], color=colors[j], label=cpmMethodLabels[i] + ": " + penetrationLabels[j])
    ...
    plt.savefig(args[1] + "_CAVpenetrarion_CPMmethod_cbr.png", dpi=300)
    plt.show()
    plt.close(fig)
