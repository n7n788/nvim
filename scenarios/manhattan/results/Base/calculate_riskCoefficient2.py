...
def calculate_riskRow(df, trueFlag):
    ...
    for row in new_df.itertuples():
        for i in range(len(row.risk)):
            if (row.distance[i] < 100) & (row.sensing[i] == 0) & (row.connected[i] == 0):
                remainder = 0
                if row.risk[i] > 1.0 + 0.01 or row.risk[i] < min_risk - 0.01:
    ...
    r = min_risk            
    for i in range(risk_num):
        bins["mean_aoi"].append(statistics.median(bins["aoi"][i]))
        bins["max_aoi"].append(max(bins["aoi"][i]))
        if i % 10 == 0:
...
bins = [calculate_riskRow(df1, False), calculate_riskRow(df2, False), calculate_riskRow(df1, True), calculate_riskRow(df2, True)]
# array = range(-10, 12, 1)
...
fig, ax = plt.subplots()
for i in range(3):
    ax.plot(risk_x[i], risk_y[i], label=labels[i])
ax.legend()
...
plt.savefig(args[1] + "_riskCoefficient2_pdf.png", dpi=300)
plt.close()
...
ax.grid()
plt.title("PDF of risk coefficient", y=-1.0)
plt.savefig(args[1] + "_riskCoefficient2_limited_pdf.png", dpi=300)
plt.close()
...
plt.savefig(args[1] + "_riskCoefficient2_cdf.png", dpi=300)
plt.close()
...
ax.grid()
plt.title("CDF of risk coefficient", y=-1.0)
plt.savefig(args[1] + "_riskCoefficient2_limited_cdf.png", dpi=300)
plt.close()
...
plt.savefig(args[1] + "_riskCoefficient2_accel_pdf.png", dpi=300)
plt.close()
...
plt.savefig(args[1] + "_riskCoefficient2_accel_cdf.png", dpi=300)
plt.close()
...
plt.savefig(args[1] + "_riskCoefficient2_decel_pdf.png", dpi=300)
plt.close()
...
ax.grid()
plt.title("PDF of risk coefficient = [-1.0, -0.1]", y=-1.0)
plt.savefig(args[1] + "_riskCoefficient2_decel_cdf.png", dpi=300)
plt.close()
...
for i in range(3):
    ...
    print(len(bins[i]["max_risk"]))
fig, ax = plt.subplots()
ax.bar(x, y, label="Risk Coefficient = 1.1", tick_label=labels, color=colors)
ax.legend(loc='lower right')
ax.tick_params(direction="in")
...
plt.savefig(args[1] + "_riskCoefficient2_max.png", dpi=300)
plt.close()
...
fig, ax = plt.subplots()
ax.bar(x, y, label="Risk Coefficient = 0.0", tick_label=labels, color=colors)
ax.legend(loc='lower right')
ax.tick_params(direction="in")
ax.set_xlabel("Cpm generation method")
ax.set_ylabel("Count")
...
plt.savefig(args[1] + "_riskCoefficient2_zero.png", dpi=300)
plt.close()
...
fig, ax = plt.subplots()
ax.bar(x, y, label="Risk Coefficient = [0,1, 1.1]", tick_label=labels, color=colors)
ax.legend(loc='lower right')
ax.tick_params(direction="in")
ax.set_xlabel("Cpm generation method")
ax.set_ylabel("Count")
...
plt.savefig(args[1] + "_riskCoefficient2_accel.png", dpi=300)
plt.close()
...
fig, ax = plt.subplots()
ax.bar(x, y, label="Risk Coefficient = [-1.0, -0.1]", tick_label=labels, color=colors)
ax.legend(loc='lower right')
ax.tick_params(direction="in")
ax.set_xlabel("Cpm generation method")
ax.set_ylabel("Count")
...
plt.savefig(args[1] + "_riskCoefficient2_decel.png", dpi=300)
plt.close()


# 真の危険係数×AoIの箱ひげ図をそれぞれ表示
fig, ax = plt.subplots()
ax.boxplot(bins[2]["aoi"], whis=1e100)
ax.legend()
...
plt.savefig(args[1] + "_aoiRiskCoefficient2.png", dpi=300)
plt.close()

fig, ax = plt.subplots()
ax.boxplot(bins[3]["aoi"], whis=1e100)
ax.legend()
ax.set_xlim([0, risk_num + 1])
ax.set_ylim([0, max_aoi + 0.1])
ax.tick_params(direction="in")
...
ax.grid()
plt.title("AoI at each risk coefficient", y=-1.0)
plt.savefig(args[2] + "_aoiRiskCoefficient2.png", dpi=300)
plt.close()
...
fig, ax = plt.subplots()
for i in range(2):
    ax.plot(x, bins[i + 2]["max_aoi"], "--", label=labels[i] + "_max", color=colors[i])
    ax.plot(x, bins[i + 2]["mean_aoi"], "-", label=labels[i] + "_mean", color=colors[i])
ax.legend()
ax.set_xlim([min_risk, max_risk])
ax.set_ylim([0, max_aoi + 0.1])
ax.tick_params(direction="in")
...
ax.grid()
plt.title("AoI at each risk coefficient", y=-1.0)
plt.savefig(args[1] + "_aoiRiskCoefficient2_max.png", dpi=300)
plt.close()
...
# for i in range(2):
#     y.append([])
# for i in range(len(bins[2]["aoi"])):
#     y[0].append(np.var(bins[2]["aoi"][i]))
# for i in range(len(bins[3])["aoi"]):
#     y[1].append(np.var(bins[3]["aoi"][i]))
# colors = ["blue", "orange"]
# fig, ax = plt.subplots()
# for i in range(2):
...
# ax.grid()
# plt.title("AoI at each risk coefficient", y=-1.0)
# plt.savefig(args[1] + "_aoiRiskCoefficient2_var.png", dpi=300)
# plt.close()
